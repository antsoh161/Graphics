...
{ 
   ...
   for (int i = 0; i < NUM_TRIANGLES; ++i) {
      ...
      if (d>0 && d<=I.t) {
        ...
        if(brdf_swapper == 0) //Lambert
          ...
          I.material.color_brdf = cook_torrance_brdf(I.light_dir,I.view_dir,I.normal, scene.triangles[i].material) * incident_angle*50;
      }
  ...
  if (I.t>1e20){
    I.point = ray.dir*I.t;
    I.normal = -ray.dir; for (int i = 0; i < NUM_SPHERES; ++i) {
    float d = intersect(ray, scene.spheres[i]);
    if (d>0 && d<=I.t) {
      I.t = d; 
      id = i;
